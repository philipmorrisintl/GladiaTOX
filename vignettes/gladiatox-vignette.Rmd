...
vignette: >
  %\VignetteIndexEntry{GladiaTOX: R Package for Processing High Content 
    Screening data}
...
The database already contains fully processed study, with
...
new database location. The following command assumes the new database is
installed at `my_folder_path`
```{r, eval=FALSE, echo=TRUE, include=TRUE}
gtoxConf(drvr = "SQLite",
         host = NA,
         user = NA,
...
```{r, eval=TRUE, fig=TRUE, fig.width=12, fig.height=8, dev=c('svg'),
    fig.cap='Example of heatmap of a plate raw values.
...
```
```{r, eval=TRUE, fig.width=14, fig.height=8, dev=c('svg'),
    fig.cap='Example of Positive control (chlorambucil chemical)
    plot with three concentrations, and three technical replicates
    per concentration.'}
...
in the study.
```{r, eval=FALSE, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE,
    results='hide'}
## Processing report
...
```{r, eval=TRUE, fig.width=8, fig.height=5, dev=c('svg'),
    fig.cap='Example of dose-response curves.'}
...
dev.off()
```
```{r, echo=FALSE, fig.width=8, fig.height=5, dev=c('svg'),
    fig.cap='Example of MEC barplot.'}
...
```{r, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, fig.width=10,
    fig.height=6, dev=c('svg'), fig.cap='Example of pie plots.
...
```{r, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, fig.width=10,
    fig.height=6, dev=c('svg'), fig.cap='Example of severity score plot.'}
glPlotToxInd(asid)
```
