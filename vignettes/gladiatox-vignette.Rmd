...
tcpl:::tcplWriteData(dat[ , list(acid, waid, wllq, rval)], lvl = 0, type = "mc")
```
...
Metadata and data are now registered in the database. Next step is to select the processing methods we want to apply on the data. There are multiple levels of processing (see `tcplMthdLoad(lvl=3)` foe details). The fucntion `assignDefaultMthds` is a shortcut to assign all levels methods at once. The methods selected would probably fit well to most users.
...
res <- tcplRun(asid = asid, slvl = 1, elvl = 3, mc.cores = 2)
...
as.integer(aeids))
res <- tcplRun(asid = asid, slvl = 1, elvl = 6, mc.cores = 2)
```
...
res <- tcplRun(asid = asid, slvl = 1, elvl = 6, mc.cores = 2)
```
The `tcplRun` returns a list of vectors of logical values used to check processing status. The resulting processed data is automatically stored in the database along with the statistics computed. Statistics include activity concentrations (AC10 and AC50) and minimal effective concentrations (MECs).
