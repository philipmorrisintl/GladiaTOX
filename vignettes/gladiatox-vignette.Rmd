...
(`sql/gladiatoxdb.sqlite` folder). The database already contains fully processed 
study, with
...
following example In case you plan to use the package for multiple studies we 
recommend to install the MySQL database (`sql/gladiatoxdb_structure.mysql`).

## Database configuration
The first step after database deployment is to configuration access parameters. 
The default configuration already points to the sqlite database at 
...
database, then you must run the configuration command below and point to the new 
database location. The following command assumes the new database is installed 
at `my_folder_path`
```{r, eval=FALSE, echo=TRUE, include=TRUE}
gtoxConf(drvr = "SQLite",
         host = NA,
...
```
In case you deploy the database schema provided in the sql folder, then change 
the driver (drvr) to `MySQL`. Eventually you may need to configure your user 
name and password.
...
In this section we will explore one simple way for loading data in the database. 
The following chunks prepare the metadata and data in R objects (data.frame) 
prior database loading.

The following commands loads the data for vignette, and set the output 
directory, `outdir`, to the working directory. Change it accordingly to the 
folder path where your reports have to be saved.
```{r, eval=FALSE, include=FALSE, echo=TRUE}
load(system.file("extdata", "data_for_vignette.rda", package="GladiaTOX"))
outdir <- getwd()
```
The command loads three objects in the environment. These objects are what users 
need to prepare before study data can be loaded.
...
treatment, `n` is the negative control); `enpoint` contains assay names with no 
...
including: assays, endpoint, treatments and controls. The status of the assay 
...
```
Then the raw data is loaded in the database with the `gtoxWriteData` function. 
...
of processing (see `gtoxLoadMthd(lvl=3)` foe details). The fucntion 
...
active versus those that are not. The default behaviour is to compute noise band
margins separately for each endpoint. Margins correspond to 3 times the baseline
median absolute deviation of vehicle responses. The following code computes the
cutoffs and store them in the database.
