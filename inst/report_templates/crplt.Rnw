...
<<<%= paste0("stim", stim, "acid", ac) %>,fig=true,strip.white=true,echo=false,height=4,width=5,include=false>>=

ae_vec <- c(<%= aes[acid == ac, paste(aeid, collapse = ", ")] %>)
tcplPlotWin(chid = <%= stim %>, aeid = ae_vec, collapse = TRUE, bline = "coff")
