...
loadAnnot <- function(plate, assay, outFile="out.json") {
    ...
    tstamp <- as.integer(Sys.time()) # define timestamp

    ## Add Human Readable Barcode if not in input
    if (is.null(plate$hr_barcode)) {
        plate[ , hr_barcode := paste(substr(asnm, 1, 7), "_",
    ...
    assay <- merge(assay,
                   data.table(tu=c("m", "h", "d", "min", "w", "b", "y"),
                              tmult=c(1/60, 1, 24, 1/60, 120, 3600, 1314000)),
                   by="tu", all.x=TRUE)
    assay[ , timepoint_hr := as.numeric(timepoint_hr)*tmult]

    ## Register the assays
    assay <- merge(unique(plate[ , list(asnm, asph, anm, assay_footprint)]),
                   assay,
                   by="anm")
    assay <- merge(
        assay,
    ...
    tcplRegister("aid",
                 unique(assay[ ,
                              list(asid, anm, timepoint_hr, assay_footprint)]))
    assay <- merge(assay,
    ...
    tcplRegister("acid", unique(assay[ , list(aid, acnm, machine_name)]))
    assay <- merge(assay,
                   unique(tcplLoadAcid(fld="asid",
                                       assay$asid)[ , list(acid, acnm)]),
                   by="acnm")
    assay <- assay[order(gsub("_[0-9]+h", "", aenm), timepoint_hr)]
    assay[ , analysis_direction := ifelse(grepl('_dn$', aenm), "down", "up")]
    assay[ , c("burst_assay", "fit_all") := list(0, 0)]
    tcplRegister("aeid",
                 unique(assay[ , list(acid, aenm, burst_assay, fit_all,
                                      analysis_direction)]))
    ...
    tcplRegister("apid",
                 unique(plate[ ,
                              list(aid, date_treat, date_harvest, hr_barcode)]))
    plate <- merge(plate,
                   tcplLoadApid()[ , list(apid, hr_barcode)],
                   by="hr_barcode")
    ...
    tcplRegister("waid",
                 unique(plate[ , list(apid, rowi, coli, spid, wllt, vhid,
                                      conc, gui_sample_id)]))
    waid <- tcplLoadWaid(fld="apid", plate$apid)
