...
.buildAssayTab <- function(plate.mtd, chn.map) {
    ...
    tab <- as.data.frame(
        unique(cbind(as.character(plate.mtd$`endpoint`),
    ...
    at$`endpoint` <- paste(at$`component`, c(rep("up",dim(tab)[1]),
                                             rep("dn",dim(tab)[1])), sep="_")
    at$`channel` <- rep(tab$Channel, 2)
    at <- data.frame(at, check.names=FALSE)
    at <- at[sort.int(as.character(at$component), decreasing=TRUE,
                      index.return=TRUE)$ix, ]

    ## remove not compatible aeids
    at <- at[!(grepl("Cell count",
                    at$endpoint) & grepl("_up", at$endpoint)), ]
    at <- at[!(grepl("DNA damage (pH2AX)",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
    at <- at[!(grepl("Stress kinase (c-Jun)",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
    at <- at[!(grepl("Caspase 3/7 intensity",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
    at <- at[!(grepl("Cytochrome C release",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
    at <- at[!(grepl("Cytotoxicity (TIER1)_Cell membrane permeability",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
    at <- at[!(grepl("NF-kB nuclear content",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
    at <- at[!(grepl("Apo Necro (casp37)_Cell membrane permeability",
                    at$endpoint, fixed=TRUE) & grepl("_dn", at$endpoint)), ]
}
