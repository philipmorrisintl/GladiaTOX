...
#-------------------------------------------------------------------------------
# tcplLoadData: Load gtox data
#-------------------------------------------------------------------------------

#' @title Load gtox data
#'
#' @description
#' \code{tcplLoadData} queries the gtox databases and returns a data.table with
...
tcplLoadData <- function(lvl, fld=NULL, val=NULL, type="mc") {
    ...
    if (!is.null(fld)) {
        ...
        fld <- tcpl:::.prepField(fld=fld, tbl=tbls, db=options()$TCPL_DB)
    ...
    dat <- suppressWarnings(tcplQuery(query=qstring, db=options()$TCPL_DB))
