...
tcplRun <- function(asid=NULL, slvl, elvl, id=NULL, type="mc",
    ...
    if (type == "mc") {
        ...
        if (slvl <= 1L) {
            res$l1 <- tcpl:::.multProc(id=id, lvl=1L, type="mc", ncores=ncores)
        ...
        if (slvl <= 2L) {
            res$l2 <- tcpl:::.multProc(id=id, lvl=2L, type="mc", ncores=ncores)
        ...
        if (slvl <= 3L) {
            res$l3 <- tcpl:::.multProc(id=id, lvl=3L, type="mc", ncores=ncores)
        ...
        if (slvl <= 4L) {
            res$l4 <- tcpl:::.multProc(id=id, lvl=4L, type="mc", ncores=ncores)
        ...
        if (slvl <= 5L) {
            res$l5 <- tcpl:::.multProc(id=id, lvl=5L, type="mc", ncores=ncores)
        ...
        if (slvl <= 6L) {
            res$l6 <- tcpl:::.multProc(id=id, lvl=6L, type="mc", ncores=ncores)
    ...
    if (type == "sc") {
        ...
        ## Do level 1 processing
        if (slvl <= 1L) {
            res$l1 <- tcpl:::.multProc(id=id, lvl=1L, type="sc", ncores=ncores)
            res$l1_failed <- names(which(res$l1 != TRUE))
            id <- id[which(res$l1[names(id)] == TRUE)]
            if (length(id) == 0) {
                warning(
        ...
        ## Do level 2 processing
        if (slvl <= 2L) {
            res$l2 <- tcpl:::.multProc(id=id, lvl=2L, type="sc", ncores=ncores)
            res$l2_failed <- names(which(res$l2 != TRUE))
            id <- id[which(res$l2[names(id)] == TRUE)]
            if (length(id) == 0) {
                warning(
