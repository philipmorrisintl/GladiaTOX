...
tcplRegister <- function(what, flds) {
    ...
    valid_what <- c("asid",  "aid", "acid", "aeid", "spid",
    ...
    i <- switch(what,
                asid=list("assay_source",
                            c("assay_source_name", "assay_source_phase")),
                aid =list("assay", c("asid", "assay_name",
                                       "assay_footprint")),
                acid=list("assay_component",
                            c("aid", "assay_component_name", "machine_name")),
                aeid=list("assay_component_endpoint",
                            c("acid",
                              "assay_component_endpoint_name")),
                spid=list("sample", c("spid", "chid")),
                chid=list("chemical", c("chnm")),
                clib=list("chemical_library", c("chid", "clib")),
                vehicle=list("vehicle", c("vehicle_name")),
                apid=list("assay_plate", c("aid")),
                waid=list("assay_plate_well",
                            c("apid", "spid", "rowi", "coli", "wllt",
                              "vhid", "conc")),
                bb_waid=list("bb_waid_map",
                               c("waid", "s_sampleid")),
                bb_apid=list("bb_apid_map",
                               c("apid", "u_boxtrack")))

    pot_flds <- tcplListFlds(tbl=i[[1]], db=options()$TCPL_DB)
