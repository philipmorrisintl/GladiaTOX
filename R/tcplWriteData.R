...
#-------------------------------------------------------------------------------
# tcplWriteData: Write screening data into the gtox databases
#-------------------------------------------------------------------------------
...
tcplWriteData <- function(dat, lvl, type) {
    ...
    if (length(ids) >= 500) {
        ...
        lapply(ibins, function(x) tcpl:::tcplCascade(lvl=lvl, type=type, id=x))

    } else {

        tcpl:::tcplCascade(lvl=lvl, type=type, id=ids)
